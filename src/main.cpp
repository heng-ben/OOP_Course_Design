...
void friendMenu(Platform_Zhb& p, int uid, int svc)
{
    ...
    while (true)
    {
        ...
        if (c == 1 || c == 2 || c == 4)
        {
            ...
            else if (c == 2) mgr.removeFriend(uid, other) ? cout << "已删除好友 " << other << "\n"
                                                          : cout << "你们还不是好友。\n";
            else
            {
        ...
        {
            int other = (svc == AS_QQ) ? AS_WECHAT : AS_QQ;   // 从另一张图开采
            vector<int> rec = mgr.recommendFriendsByOtherService(uid, other);
            cout << "推荐好友(" << serviceName(other) << "中还不是"
                 << serviceName(svc) << "好友的): ";
            if (rec.empty()) cout << "(无)";
...
void userMenu(Platform_Zhb& p, int uid)
{
    ...
    while (true)
    {
        ...
        if (c == 1)
        {
            // 好友操作需要一个 QQ/微信 图:优先QQ,否则微信,都没有则提示开通
            vector<int> op = p.getOpenedServices(uid);
            int svc = -1;
            for (size_t i = 0; i < op.size(); ++i)
                if (op[i] == AS_QQ)       { svc = AS_QQ; break; }
                else if (op[i] == AS_WECHAT) { svc = AS_WECHAT; break; }
            if (svc == -1)
                cout << "请先在主菜单开通 QQ 或 微信 后再进行好友管理。\n";
...
int main()
{
    ...
    while (true)
    {
        ...
        if (c == 1)   // ---------- 登录 ----------
        {
            cout << "请输入用户ID: ";
            int id = readInt();
            if (platform.findUser(id) == nullptr) { cout << "该用户不存在,请先确认ID。\n"; continue; }

            platform.login(id);
            current = id;
            cout << "登录成功。";

            // 自动遍历已开通的所有微X服务 → 全部在线;若未开通则询问是否立即开通
